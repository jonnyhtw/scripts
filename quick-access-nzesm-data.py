...
for j in range(len(rawstashs)):
    ...
    for i in range(len(months)):
        ...
        infile = suite[2:]+'a.p'+streams[i]+str(year)+months[i]+'.pp'

        outdir ='/nesi/project/niwa00013/williamsjh/NZESM/quick-access-nzesm-data/'+suite+'/' 

        outfile = 'm'+stashmodel+'s'+rawstashs[j][0:2]+'i'+rawstashs[j][2:]+'-'+descriptions[j]+'-'+suite[2:]+'a.p'+streams[i]+str(year)+months[i]+'.pp'
